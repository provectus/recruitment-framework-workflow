...
- **Networking:** A dedicated VPC with public and private subnets across at least 2 availability zones. The API and database run in private subnets; only the load balancer and CloudFront are publicly accessible.
  - **Acceptance Criteria:**
    - [x] VPC is created with public and private subnets in 2+ AZs
    - [x] Internet Gateway and NAT Gateway are provisioned for outbound access from private subnets
    - [x] Security groups restrict access: ALB accepts 443 inbound; ECS tasks accept traffic only from ALB; RDS accepts connections only from ECS security group

- **Compute (ECS Fargate):** The FastAPI backend runs as a Fargate service behind an Application Load Balancer.
  - **Acceptance Criteria:**
    - [x] ECS cluster with a Fargate service running the backend container
    - [x] Application Load Balancer routes HTTPS traffic to the ECS service
    - [x] Health check endpoint (`/health`) returns 200 through the ALB
    - [x] Service auto-restarts on container failure

- **Database (RDS PostgreSQL):** A managed PostgreSQL instance for application data.
  - **Acceptance Criteria:**
    - [x] RDS PostgreSQL instance is running in a private subnet
    - [x] ECS tasks can connect to the database
    - [x] Automated backups are enabled
    - [x] Database credentials are stored in AWS Secrets Manager (not hardcoded)

- **Storage (S3):** Buckets for SPA static hosting and future file uploads.
  - **Acceptance Criteria:**
    - [x] S3 bucket for React SPA static assets, served via CloudFront
    - [x] S3 bucket for application file storage (CVs, transcripts — future use)
    - [x] Buckets are not publicly accessible (access only via CloudFront OAC or presigned URLs)

- **CDN (CloudFront):** Serves the React SPA globally with HTTPS.
  - **Acceptance Criteria:**
    - [x] CloudFront distribution serves the SPA from S3
    - [x] SPA routes (e.g., `/login`, `/candidates`) return `index.html` (SPA fallback behavior)
    - [x] API requests (`/api/*`) are proxied to the ALB backend — OR — frontend calls the API domain directly via CORS [NEEDS CLARIFICATION: proxy through CloudFront vs separate API domain — to be decided in tech spec]

- **AI (Bedrock):** Claude model access for future evaluation features.
  - **Acceptance Criteria:**
    - [x] Bedrock model access is enabled for Claude in us-east-1
    - [x] An IAM role exists that ECS tasks can assume to call Bedrock APIs
...
- **Authentication (Cognito):** AWS Cognito User Pool with Google as federated identity provider, powering the existing OAuth 2.0 login flow.
  - **Acceptance Criteria:**
    - [x] Cognito User Pool is created with Google as a federated identity provider
    - [x] App client is configured for OAuth 2.0 authorization code grant with appropriate callback URLs (`https://api.<domain>/auth/callback`)
    - [x] Hosted UI domain is provisioned for the Google sign-in redirect
    - [x] Cognito client ID, client secret, and pool ID are stored in Secrets Manager / SSM and injected into ECS task env vars

- **Monitoring (CloudWatch):** Basic observability for the POC.
  - **Acceptance Criteria:**
    - [x] ECS service logs are shipped to CloudWatch Logs
    - [x] ALB access logs are enabled
    - [x] Basic CloudWatch alarms: ECS service unhealthy, RDS CPU > 80%
...
- **Acceptance Criteria:**
  - [x] ACM certificate issued and validated for the chosen domain
  - [x] CloudFront distribution uses the custom domain with HTTPS
  - [x] API is accessible via a subdomain (e.g., `api.<domain>`) or path prefix — with valid HTTPS
  - [x] DNS records are created (domain-agnostic — specific domain TBD)
...
- **Acceptance Criteria:**
  - [x] Push to `main` triggers deployment of both backend and frontend
  - [x] Backend pipeline: lint → test → build Docker image → push to ECR → deploy to ECS
  - [x] Frontend pipeline: lint → build → sync to S3 → invalidate CloudFront cache
  - [x] Pipeline uses OIDC-based AWS authentication (no long-lived access keys)
  - [x] Failed lint or tests block deployment
  - [x] Deployment status is visible in GitHub (check status on PRs)
...
- **Acceptance Criteria:**
  - [x] All resources above are defined in Terraform configurations
  - [x] Terraform state is stored remotely (S3 + DynamoDB locking)
  - [x] `terraform plan` produces a clean diff from a fresh state
  - [x] Terraform configurations are in the repository (e.g., `infra/` directory)
